# Run Summary: 2026-05-09_weekly

Generated: 2026-05-04

## Metrics

- provider_tasks_planned: 10
- analysis_tasks_planned: 4
- evidence_packets: 17
- financial_reviews: 1
- company_news_reviews: 1
- recommended_updates: 3
- risks: 2
- contradictions: 0
- unknowns: 0

## Tracked Tickers

- current_holdings: none
- monitoring: AAPL
- rejected: none

## Provider Packet Counts

- alpha_vantage: 1
- company_news_specialist: 1
- exa: 6
- financial_compare: 1
- financial_data_specialist: 1
- fmp: 1
- polygon: 1
- sec_edgar: 1
- xai_grok: 3
- yfinance: 1

## Financial Reviews

| Ticker | Status | Report |
| --- | --- | --- |
| AAPL | ready_for_company_update | `agents/runs/2026-05-09_weekly/reports/financial_data_specialist/AAPL_financial_review.md` |

## Company News Reviews

| Ticker | Status | Report |
| --- | --- | --- |
| AAPL | ready_for_company_update | `agents/runs/2026-05-09_weekly/reports/company_news_specialist/AAPL_company_news_review.md` |

## Open Items

- No deterministic open items found.
