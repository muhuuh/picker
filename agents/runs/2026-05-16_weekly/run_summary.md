# Run Summary: 2026-05-16_weekly

Generated: 2026-05-16

## Metrics

- provider_tasks_planned: 81
- analysis_tasks_planned: 50
- evidence_packets: 131
- financial_reviews: 10
- company_news_reviews: 10
- recommended_updates: 30
- risks: 31
- contradictions: 0
- unknowns: 266

## Tracked Tickers

- current_holdings: AXTI, NBIS, MU, GOOGL, IREN, OSS, TE, AMBA, AVAV, KRKNF
- monitoring: none
- rejected: none

## Provider Packet Counts

- alpha_vantage: 10
- company_news_specialist: 10
- exa: 32
- financial_compare: 10
- financial_data_specialist: 10
- fmp: 10
- opportunity_assessment_specialist: 10
- polygon: 9
- sec_edgar: 9
- xai_grok: 11
- yfinance: 10

## Financial Reviews

| Ticker | Status | Report |
| --- | --- | --- |
| AMBA | ready_for_company_update | `` |
| AVAV | ready_for_company_update | `` |
| AXTI | ready_for_company_update | `` |
| GOOGL | ready_for_company_update | `` |
| IREN | ready_for_company_update | `` |
| KRKNF | partial_review | `` |
| MU | ready_for_company_update | `` |
| NBIS | ready_for_company_update | `` |
| OSS | ready_for_company_update | `` |
| TE | ready_for_company_update | `` |

## Company News Reviews

| Ticker | Status | Report |
| --- | --- | --- |
| AMBA | ready_for_company_update | `` |
| AVAV | ready_for_company_update | `` |
| AXTI | ready_for_company_update | `` |
| GOOGL | ready_for_company_update | `` |
| IREN | ready_for_company_update | `` |
| KRKNF | ready_for_company_update | `` |
| MU | ready_for_company_update | `` |
| NBIS | ready_for_company_update | `` |
| OSS | ready_for_company_update | `` |
| TE | ready_for_company_update | `` |

## Open Items

- No deterministic open items found.
