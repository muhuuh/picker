...
def main(argv: list[str] | None = None) -> int:
    ...
    agent_runtime_run.add_argument("--subject-type", default="theme", help="Subject type for market_research_orchestrator prompts.")
    agent_runtime_run.add_argument("--subject-id", help="Subject id for market_research_orchestrator prompts.")
    agent_runtime_run.add_argument("--topic", help="Human-readable topic for market_research_orchestrator prompts.")
    ...
    run_weekly_parser.add_argument("--execute-orchestrator", action="store_true", help="Optional API mode: call OpenAI Agents SDK orchestrator after deterministic finalization. Requires --write and OPENAI_API_KEY.")
    ...
    if args.command == "agent-runtime":
        ...
        if args.agent_runtime_command == "run":
            ...
            elif args.agent_id == "market_research_orchestrator" and (args.subject_id or args.topic):
                prompt = build_market_research_input(
                    context,
                    args.subject_id or "",
                    args.subject_type,
                    args.topic or "",
