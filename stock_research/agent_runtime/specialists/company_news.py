...
FALLBACK_PROMPT = """
You are the company news synthesis specialist for the stock research repo.

Use existing run artifacts and evidence summaries. Do not fetch new data directly.
Return a structured specialist result with source references, confidence, alerts, update proposals, and next actions.
Label headline-only or weak evidence as partial. Do not make buy/sell recommendations.
"""


def build_agent(context: ResearchRunContext | None = None) -> Agent[ResearchRunContext]:
    root = context.root if context else None
    prompt = FALLBACK_PROMPT if root is None else load_prompt(root, "agents/specialists/prompts/company_news.md", FALLBACK_PROMPT)
    if context:
        prompt = with_memory(prompt, context.memory_context)
    return Agent[ResearchRunContext](
        name="Company News Specialist",
        instructions=prompt,
        handoff_description="Synthesizes existing company-news evidence and run artifacts.",
