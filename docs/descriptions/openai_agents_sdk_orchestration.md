...
- `stock_research/opportunity_assessment.py`: deterministic opportunity assessment over financials, news, SEC filings, Exa context, Grok/X sentiment, and risk signals. This is the deeper human-facing company report and includes investor insight sections for company/industry context, thesis change, Grok/X community split, non-obvious insights, valuation/analyst target context, peer context, decision table, and next research questions.
- `stock_research/weekly_digest.py`: deterministic final digest writer for concise run-level review summaries that link to the deeper opportunity reports.
...
Default Codex-supervised scheduled mode:

```powershell
C:\Python313\python.exe -m stock_research run-weekly --write --execute-providers --execute-analysis
```

Then Codex reads `agents/runs/{run_id}/codex_supervised_review_pack.md` and writes `agents/runs/{run_id}/codex_supervised_review.md`.
...
Weekly-style runs also write `agents/runs/{run_id}/final_digest.md`. This is the primary concise run-level triage report for humans when the live SDK main orchestrator is unavailable, too noisy, or still pending validation. It must not be a status dashboard. It should answer what changed, why it matters, what X/community narratives are forming, which accounts/posts are worth reviewing, what is verified vs social/speculative, and what research decision follows. It includes per-ticker opportunity view, readable key financial facts, explicit evidence links, concrete Exa/news developments, Grok/X pulse, recurring bull/bear narratives, hype/noise, filing coverage, next research checks, and next actions. Digest validation flags missing core evidence links, non-social Grok/X labels, status-only Grok/X output, missing bull/bear social claims, direct trade language, and missing financial/news review status before the report can be treated as ready.
