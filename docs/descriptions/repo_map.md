...
- `python -m stock_research market-research candidate-review --run-id RUN_ID --write --queue-review`: group manual market-research candidate leads, including duplicate listings/share classes and repeated Grok/X multi-ticker baskets, write `candidate_review.md`, and append duplicate-safe human-review queue rows. Regenerated open rows for the same run are marked `superseded`. This does not add stocks to monitoring.
