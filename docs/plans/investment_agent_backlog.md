...
- [ ] Run prompt/output quality iteration on realistic manual and weekly-style examples.
  ...
  - Next report-shape gap: run fresh live Grok/X output through the strengthened prompts for both one company and one industry/theme, then improve prompts/extraction where the new output still feels generic or misses non-obvious insight.
- [ ] Add golden/failure tests.
  ...
  - Gap: extend golden fixture cases with stronger examples modeled on the user-provided Grok PDFs, especially fresh live Grok/X industry discovery and company sentiment runs.
